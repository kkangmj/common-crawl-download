...
public class Deduplicate {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (Long sizesKey : sizesKeys) {
            ...
            for (String file : sizeFiles) {
                count++;
                try {
                    String hash = hash(new File(Utils.DOWNLOAD_DIR, file));
                    if (hashes.containsKey(hash)) {
                        duplicates++;
                        log.info("Dups: " + duplicates + ", Count: " + count + ", SizeKey: " + sizesKey +
                                ": File " + file + " is the same as " + hashes.get(hash));

                        FileUtils.moveFile(new File(Utils.DOWNLOAD_DIR, file), new File(Utils.BACKUP_DIR, file));
                    } else {
                        hashes.put(hash, file);
                    }
                } catch (FileNotFoundException e) {
                    log.log(Level.WARNING, "Could not read file '" + new File(Utils.DOWNLOAD_DIR, file).getAbsolutePath() +
                            "' for size " + sizesKey + ", probably the filename contains unexpected characters", e);
                } catch (IOException | RuntimeException e) {
                    throw new IOException("Failed for file '" + new File(Utils.DOWNLOAD_DIR, file).getAbsolutePath() +
                            "' for size " + sizesKey, e);
                }
            }
        }
    ...
    private static TreeMultimap<Long, String> scanAndSortFiles() throws IOException {
        ...
        Files.walkFileTree(Utils.DOWNLOAD_DIR.toPath(), EnumSet.of(FileVisitOption.FOLLOW_LINKS),
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
                ...
                sizes.put(file.toFile().length(),
                        StringUtils.removeStart(file.toFile().toString(), Utils.DOWNLOAD_DIR.toString() + "/"));
